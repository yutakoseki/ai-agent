...
function isLikelyHtmlUrl(url: string): boolean {
  ...
  return !/\.(pdf|png|jpe?g|gif|webp|svg|mp4|mp3|zip|xlsx?|pptx?)($|\?)/.test(lowered);
}

async function fetchArticleText(url: string | undefined, traceId?: string): Promise<string | null> {
  if (!url) return null;
...
function normalizePromptText(value: string | undefined, fallback: string, maxLength = 200): string {
...
function buildPostStyleContext(input: SummaryInput): { postTone: string; postFormat: string } {
  return {
    ...
    postFormat: normalizePromptMultiline(input.postFormat, DEFAULT_POST_FORMAT, MAX_POST_FORMAT),
  };
}
...
function normalizeAiTitle(raw: unknown, fallback: string): string {
  ...
  if (!value) return '要約';
  return value.length <= MAX_BLOG_TITLE ? value : `${value.slice(0, MAX_BLOG_TITLE - 1)}…`;
}
...
function summarizeForBlogLocal(input: SummaryInput): BlogSummary {
  ...
  return {
    title:
      baseTitle.length <= MAX_BLOG_TITLE ? baseTitle : `${baseTitle.slice(0, MAX_BLOG_TITLE - 1)}…`,
    text,
  };
}
...
function summarizeForXLocal(input: SummaryInput, blog?: BlogSummary): string {
  ...
  if (text.length < 140 && sentences.length > 1) {
    ...
    return combined.length <= MAX_X_LENGTH ? combined : truncateText(combined, MAX_X_LENGTH);
  }

  return text;
}

async function summarizeForBlogAI(input: SummaryInput): Promise<BlogSummary | null> {
...
async function summarizeForXAI(input: SummaryInput, blog: BlogSummary): Promise<string | null> {
...
export async function summarizeForBlog(input: SummaryInput): Promise<BlogSummary> {
...
export async function summarizeForX(input: SummaryInput & { blog?: BlogSummary }): Promise<string> {
