...
##### 注意: 統合テストは DynamoDB Local（Docker）を使用します

統合テストは DynamoDB を読み書きするため、**事前に DynamoDB Local を起動し、テーブル作成 + 初期データ投入（seed）**を行ってください。
...
- ✅ 単体テスト: auth/middleware/error など主要ユーティリティは Vitest でカバー
- ✅ 統合テスト: API ルートの基本ケースを Vitest (node 環境) で実行
- ✅ E2E スモーク: Playwright でトップページ疎通 (`apps/web/e2e/home.spec.ts`)

### 未着手/課題

- [ ] API 統合テストの網羅拡張（認可・テナント分離・エラー系）
- [ ] E2E シナリオ追加（ログイン→テナント選択→代表エージェント実行）
