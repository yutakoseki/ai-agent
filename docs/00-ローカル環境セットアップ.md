...
2. Cognito の **`sub` を確認**
3. DynamoDB に **テナント + 管理者ユーザー** を登録（シードスクリプト）

実運用の例（AWS 直書き。`DYNAMODB_ENDPOINT` は不要）:
...
## DynamoDB のテーブル構成（シングルテーブル）

- アプリ用のテーブルは **1 つだけ**（例: `aiagent-dev`）
- `aiagent-terraform-lock` は **Terraform state ロック用**で、アプリデータとは無関係
- `aiagent-user` のような **ユーザー専用テーブルは不要**
- ユーザーは同じテーブル内に `PK=TENANT#<tenantId>`, `SK=USER#<userId>` で格納
