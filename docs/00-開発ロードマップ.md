...
- [ ] テナント削除/無効化 API（DB 反映は未実装、レスポンスのみ）
...
- [x] テナント内ユーザー一覧取得 API
- [x] ユーザー役割変更 API
- [ ] Cognito ユーザー作成との同期（現状 DynamoDB のみで登録）
...
## フェーズ 2: DynamoDB 基盤強化

### 目的

シングルテーブル設計でテナント分離を保証し、Amplify から安全に利用できるようにする。

### タスク

#### 2.1 テーブル/IaC

- [x] Terraform で DynamoDB テーブル定義（PK/SK + GSI1/GSI2、TTL、PITR、KMS）
- [x] Amplify 実行ロールへの最小権限 IAM ポリシー
- [ ] Amplify Gen2 リソースを Terraform 化し、環境変数を IaC で注入

#### 2.2 アプリ実装

- [x] DynamoDB Local 用シードスクリプト（`packages/db-dynamo/scripts/seed.mjs`）
- [x] テナント/ユーザー CRUD ラッパ（`packages/db-dynamo`）
- [ ] Cognito `sub` をキーにしたユーザー同期・重複チェック
- [ ] テナント削除/無効化の永続化

#### 2.3 運用・テスト

- [ ] DynamoDB Local を使った統合テストの整備
- [ ] 監査/実行ログアイテムのスキーマ拡張とメトリクス化

### 成果物

- `infra/terraform/dynamodb/*` : DynamoDB テーブル/IAM 定義
- `infra/terraform/amplify/*` : Amplify 定義（追加予定）
- `packages/db-dynamo/*` : テーブルアクセスラッパ
- `docs/architecture/データベース設計.md` : DB 設計ドキュメント（現行反映済み）
